...
TEST(test_arma, Axby) {
  ...
  auto Axby{[](const arma::fmat &x, const arma::fmat &w, const arma::fmat &b, arma::fmat &y){
    y = w * x + b;
  }};
...
TEST(test_arma, e_power_minus) {
  ...
  auto EPowerMinus{[](const arma::fmat &x, arma::fmat &y){
    // 把代码写这里 完成y = e^{-x}的运算
    y = arma::exp(-x);
  }};
...
TEST(test_arma, axpy) {
  ...
  auto Axpy{[](const arma::fmat &x, arma::fmat &Y, float a, float y){
    // 编写Y = a * x + y
    Y = a * x + y;
  }};
